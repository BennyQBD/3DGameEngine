...
//		float rotAmt = (float)(100 * Time.getDelta());
		...
		if(mouseLocked)
		{
			...
			if(rotX)
				getTransform().setRot(getTransform().getRot().mul(new Quaternion(getTransform().getTransformedRot().getRight(), ((float) Math.toRadians(-deltaPos.getY() * sensitivity)))).normalized());
